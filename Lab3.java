...
public class Lab3 {
    ...
    static ScapegoatTree<Ngram, ArrayList<Path>> buildIndex(ScapegoatTree<Path, Ngram[]> files) {
        ...
        for (Path path: files.keys()) {
            // For each n-gram in this file
            for (Ngram ngram: files.get(path)) {
                // Get or create the list of files containing this n-gram
                ArrayList<Path> fileList = index.get(ngram);
                if (fileList == null) {
                    fileList = new ArrayList<Path>();
                    index.put(ngram, fileList);
                }
    ...
    static ScapegoatTree<PathPair, Integer> findSimilarity(ScapegoatTree<Path, Ngram[]> files, ScapegoatTree<Ngram, ArrayList<Path>> index) {
        ...
        // Iterate through all n-grams in the index
        for (Ngram ngram: index.keys()) {
            ArrayList<Path> fileList = index.get(ngram);
            // For each pair of files that contain this n-gram
            for (int i = 0; i < fileList.size(); i++) {
                for (int j = i + 1; j < fileList.size(); j++) {
                    Path path1 = fileList.get(i);
                    Path path2 = fileList.get(j);
                    // Ensure path1 > path2 (as required by findMostSimilar)
                    PathPair pair;
                    if (path1.compareTo(path2) > 0) {
                        pair = new PathPair(path1, path2);
                    } else {
                        pair = new PathPair(path2, path1);
                    }
                    
                    // Initialize or increment similarity score
